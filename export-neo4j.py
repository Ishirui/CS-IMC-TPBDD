...
with pyodbc.connect('DRIVER='+driver+';SERVER=tcp:'+server+';PORT=1433;DATABASE='+database+';UID='+username+';PWD='+ password) as conn:
    ...
    while True:
        ...
        for row in rows:
            ...
            n = Node("Film", idFilm=row[0], title=row[1], year=row[2])
            importData.append(n)
            i += 1
    ...
    while True:
        ...
        for row in rows:
            ...
            n = Node("Artist", idArtist=row[0], name=row[1], year=row[2])
            importData.append(n)
            i += 1
    ...
    while True:
        ...
        try:
            for cat in importData:
                ...
                # A COMPLETER
                create_relationships(graph.auto(), data=importData[cat], rel_type=cat.replace(" ", "_"), start_node_key=("Artist", "idArtist"), end_node_key=("Film", "idFilm"))
